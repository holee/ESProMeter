    ...
    public static class ItemController
    {
        ...
        public static bool DeleteItem(this Form form,long id)
        {
            try
            {
                ...
                instance.UseSql("DELETE FROM [dbo].[Item] WHERE ItemID=@itemID ;")
                       .Delete(new { itemID = id });
                instance.UseSql("DELETE FROM [dbo].[ITEMBOQLINE] WHERE ItemRefID=@itemID;")
                        .Delete(new { itemID = id });
        ...
        public static void ShowItemList(this Form form,DataGridView grid, int numberOfRows, string sortBy)
        {
            instance.UseSql($"SELECT TOP({numberOfRows}) * FROM [dbo].[VItem] ORDER BY {sortBy} DESC;")
               .FindAsTable<dynamic?>(null)
               .UseDataTableAsGridView(grid);
        }
        public static void ShowItemList(this Form form,int numberOfRows=50,string sortBy="ItemID")
        {
            instance.UseSql($"SELECT TOP({numberOfRows}) * FROM [dbo].[vItemWithUom] ORDER BY {sortBy} DESC;")
               .FindAsTable<dynamic?>(null)
               .UseDataTableAsGridView(form.AsControl<DataGridView>("dataItemList"));
        }
        public static void ShowItemList(this Form form,string type, int numberOfRows = 50, string sortBy = "ItemID")
        {
            instance.UseSql($"SELECT TOP({numberOfRows}) * FROM [dbo].[vItemWithUom] WHERE [Type]=@type ORDER BY {sortBy} DESC;")
               .FindAsTable<dynamic>(new { type=type })
               .UseDataTableAsGridView(form.AsControl<DataGridView>("dataItemList"));
        }
        public static void ShowItemList(this Form form, string type,string itemName,params string[] columns)
        {
            var sql = columns.Length == 0 ? "*" : string.Join(",", columns);

            instance.UseSql($"SELECT TOP(10) {sql} FROM [dbo].[vItemWithUom] WHERE [Type]=@type AND [Name] LIKE @itemName+'%';")
               .FindAsTable(new { type = type,itemName= itemName }) 
               .UseDataTableAsGridView(form.AsControl<DataGridView>("dataItemList"));
        }
        public static bool ShowItemList(this Form form,out DataTable table , string type, string itemName, params string[] columns)
        {
            var sql = columns.Length == 0 ? "*" : string.Join(",", columns);

            table = instance.UseSql($"SELECT TOP(10) {sql} FROM [dbo].[vItemWithUom] WHERE [Type]=@type AND [Name] LIKE @itemName+'%';")
               .FindAsTable(new { type = type, itemName = itemName });
            return table.Rows.Count > 0;
        }
        public static void ShowItemList(this Form form, string type, 
                                        int numberOfRows, 
                                        string sortBy,
                                        params string[] columns)
        {
            string newSql = columns.Length == 0 ? "*" : string.Join(",",columns);
            instance.UseSql($"SELECT TOP({numberOfRows}) {newSql} FROM [dbo].[vItemWithUom] WHERE [Type]=@type ORDER BY {sortBy} DESC;")
               .FindAsTable(new { type = type })
               .UseDataTableAsGridView(form.AsControl<DataGridView>("dataItemList"));
        }
