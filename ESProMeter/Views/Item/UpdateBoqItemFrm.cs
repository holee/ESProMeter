...
namespace ESProMeter.Views.Items
{
    ...
    public partial class UpdateBoqItemFrm : Form
    {
        int quantity = 0;
        string dataFound = "";
        public UpdateBoqItemFrm()
        {
            InitializeComponent();
        }
        public UpdateBoqItemFrm(long id)
        {
            InitializeComponent();
            this.ShowBoqItems();
            this.ShowUom();
        ...
        private void TextBoxMaterialQuantityKeyUp(object sender, KeyEventArgs e) 
        {
            if (!string.IsNullOrEmpty(((TextBox)sender).Text))
            {
                ...
                if (qty < 0)
                {
                    ((TextBox)sender).SetText(Utility.NumberString(0,"N3"));
                    qty = 1;
                }
        ...
        private void materialButton1_Click(object sender, EventArgs e)
        {
            ...
            {
                //var data = this.AsObjectOnly(label10,lblItemID,textName, textCost, chkIsInActive, textPrice, cmbType, cmbUom, textDescription);
                var data = new
                {
                    ItemID=lblItemID.AsNumber<long>(),
                    EditSequense=label10.AsNumber<int>(),
        ...
        private void dataLabor_CellValueChanged(object sender, DataGridViewCellEventArgs e)
       {
        ...
        {
            if (dataLabor.Columns[e.ColumnIndex].Name== "Column6")
            {
                if (!dataLabor.Rows[e.RowIndex].IsNewRow)
                {
                    var gRow = dataLabor.CurrentRow;
                    dataTemp.Rows.Add(lblItemID.AsNumber<long>(),gRow.GetValue("laborBOQItemLineRefID"));
                    dataLabor.Rows.RemoveAt(e.RowIndex);
                    //Calculate total costgro
                    CalculateCost();
                    //Calculate Additinal cost
                    CalculateAdditionalCost();
                    //calculate sale price
        ...
        {
            if (dataMachinary.Columns[e.ColumnIndex].Name == "Column7")
            {
                if (!dataMachinary.Rows[e.RowIndex].IsNewRow)
                {
                    var gRow = dataMachinary.CurrentRow;
                    dataTemp.Rows.Add(lblItemID.AsNumber<long>(), gRow.GetValue("mnrItemID"));
        ...
        private void dataMaterial_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dataMaterial.Columns[e.ColumnIndex].Name== "Column8")
            {
                ...
                {
                    var gRow = dataMaterial.CurrentRow;
                    dataTemp.Rows.Add(lblItemID.AsNumber<long>(), gRow.GetValue("mtrItemRefID"));
        ...
        private void dataMaterial_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dataMaterial.Columns[e.ColumnIndex].Name == "mtrQuantity")
            {
                ...
                {
                    dataMaterial.SetText(e.RowIndex, "mtrQuantity", Utility.NumberString(0, "N3"));
                }
                else
                {
                    var qty = dataMaterial.GetValue<decimal>(e.RowIndex, "mtrQuantity");
                    dataMaterial.SetText(e.RowIndex, "mtrQuantity",Utility.NumberString(qty,"N3"));
        ...
        private void textName_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            if (ItemController.ItemExistSame(lblItemID.AsNumber<long>(),text))
            {
